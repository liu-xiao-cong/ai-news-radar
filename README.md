...
**Cong's AI Radar 自动筛选模型、产品、开发者与产业信号，把分散消息组织成可追踪的 AI 情报流。**
...
[在线页面](https://liu-xiao-cong.github.io/ai-news-radar/) · [English](README.en.md) · [雷达Skill](skills/radar/README.md) · [伯乐Skill](skills/ai-news-radar/README.md) · [信息源策略](docs/SOURCE_COVERAGE.md)
...
**① 只想看AI日报** → 不用装任何东西，直接打开[Cong's 在线雷达](https://liu-xiao-cong.github.io/ai-news-radar/)。
...
Cong's AI Radar 是一个自动更新的 24 小时 AI 信号控制台。它不只是把 AI 新闻抓回来，会先判断信息源质量，把同一个事件合并成故事线，最后用优先级、AI 标签、源健康和 AI 占比帮你判断，
...
![Cong's AI Radar 当前四宫格截图](assets/screenshots/ai-news-radar-current-grid.jpg)
...
故事线合并、AI 标签分数、源健康与 AI 占比，仍是 Cong's AI Radar 的地基。项目版本见 [Releases](https://github.com/liu-xiao-cong/ai-news-radar/releases)。
...
- `data/history/index.json`：最近 21 个上海自然日的日期索引，供页面日期选择器使用
- `data/history/YYYY-MM-DD.json`：对应日期的 AI 强相关新闻，按需加载而不是一次加载完整归档
...
页面默认展示最新 24 小时，也可以从“查看日期”选择最近 21 天中的任意一天。所选日期会同步到 `?date=YYYY-MM-DD`，因此可以刷新、收藏或分享；历史日文件只包含 AI 强相关内容。如果 `daily-brief.json` 暂时不存在，页面会回退到候选信号列表；如果 `stories-merged.json` 存在，页面会用完整故事池补齐后续故事线，避免只有少量精选故事被接入。
...
<sub>**Cong's AI Radar** · 由 [liu-xiao-cong](https://github.com/liu-xiao-cong) 维护</sub>
